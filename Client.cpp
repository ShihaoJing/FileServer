 ...
 */
void put_file(int fd, char *put_name, int check_sum, unsigned char *aes_key, unsigned char *aes_iv)
{
	...
	int enc_msg_len = EVP_encrypt((unsigned char*)buffer, file_size, (unsigned char**)&enc_msg, aes_key, aes_iv);

	free(buffer); 

	buffer = enc_msg;
	file_size = enc_msg_len;
 ...
 */
void get_file(int fd, char *get_name, char *save_name, int check_sum, unsigned char *aes_key, unsigned char *aes_iv)
{
	/* TODO: implement a proper solution, instead of calling the echo() client */
	...
	if (strcmp(status, "OK") == 0 || strcmp(status, "OKC") == 0) {
		...
		char *dec_msg;
		int dec_msg_len = EVP_decrypt((unsigned char*)file_buffer.get_c_str(), file_size, (unsigned char**)&dec_msg, aes_key, aes_iv);

		write_file(dec_msg, dec_msg_len, save_name);
...
int main(int argc, char **argv)
{
	...
	if(put_name)
	{
		put_file(fd, put_name, check_sum, aes_key, aes_iv);
	}
	else
	{
		get_file(fd, get_name, save_name, check_sum, aes_key, aes_iv);
	}
